...
- Fixed: Docker Hub search is more reliable — it now reports the correct total number of matches, the "jump to last page" control works again, deep result pages no longer error out, and a search that finds nothing shows a clean "no matches" message instead of failing
- Added: When a Docker Hub search turns up an image that already has an app in Community Applications, its card now shows a single "Show in Apps" button — replacing the old (no-longer-working) "Docker Hub" and "Actions" buttons — that jumps straight to that app in the store. The search box shows the app's name, with its template URL on the line beneath "Clear Search", and an "ALL RESULTS" link runs a normal app search for that name
- Added: New `?templateURL=...` address-bar parameter (alongside the existing `?search=`) opens CA directly to a specific app by its template URL; matching is case-insensitive. The same jump can also be triggered from a `URL:...` link placed in an app's Additional Requirements text
