...
- Changed: When the browser can't reach the server (a network hiccup, or a stale security token after the server has been rebooted), Community Applications now shows a reload banner instead of a popup
- Changed: That reload banner now explains what went wrong — it names the request that failed and shows the last couple of lines from the PHP error log — instead of just saying something went wrong
- Changed: The reload banner now has clear Ignore and Reload page buttons instead of reloading when you click anywhere, so a stray click can't throw away the page
- Added: A "Download logs" button on the reload banner, so you can save Community Applications' logs when something goes wrong without leaving the page
...
- Fixed: The "Home" menu item is now highlighted while you are on the Community Applications home page
