...
- Changed: In a Docker Hub search, dragging the scrollbar now jumps straight to that page instead of paging through one screenful at a time to get there, and results are capped at 100 pages (Docker Hub's search API doesn't serve reliably beyond that, which previously left it stuck re-fetching the last page)
