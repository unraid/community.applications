...
- Changed: That banner now explains what went wrong — it names the request that failed and shows the last couple of lines from the PHP error log — instead of just saying something went wrong
- Changed: That banner now has clear Ignore and Reload page buttons, so a stray click can't throw away the page
- Added: A "Download logs" button on that banner, so you can save Community Applications' logs when something goes wrong without leaving the page
