...
/**
 * Bundled XML <-> array libraries (TypeConverter, Array2XML, XML2Array).
 *
 * Lazy-loaded by helpers.php via spl_autoload_register — the file is only
 * parsed when one of these class names is first referenced (today: readXmlFile
 * and include/diff.php). Splitting it out keeps the day-to-day request path
 * from parsing ~800 lines of unused third-party code.
 */
...
class TypeConverter {
	...
	public static function isSerialized($data) {
		...
		/* allowed_classes => false neutralizes PHP object injection if this
		   ever runs against untrusted bytes (callers route TypeConverter::
		   toArray/toJson/toObject through here for format detection). */
		$ser = @unserialize($data, ['allowed_classes' => false]);
	...
	public static function isXml($data) {
		...
		/* LIBXML_NONET hardens against external-entity network resolution
		   (XXE defense-in-depth) — these libraries sit on the remote-XML
		   diff path so a hostile template can't trigger an outbound DTD
		   fetch on permissive libxml builds. */
	...
	public static function xmlToArray($xml, $format = self::XML_GROUP) {
		if (is_string($xml)) {
			/* LIBXML_NONET — same XXE hardening as isXml() above. */
...
class XML2Array {
	...
	private static $encoding = 'UTF-8';
		...
		if(is_string($input_xml)) {
			/* LIBXML_NONET — same XXE hardening as TypeConverter above. */
			$parsed = $xml->loadXML($input_xml, LIBXML_NONET);
			if(!$parsed) {
