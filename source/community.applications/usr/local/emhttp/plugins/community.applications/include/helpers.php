 ...
 */
/**
 * The Home startup "Show More" sections (everything except the special Featured
 * row), used by the Home startup renderer (handleHomeStartupDisplay). The
 * left-menu Home submenu in skin.html hard-codes the same entries (it can't loop
 * this list because tr() echoes during page render) — keep the two in sync.
 * Each entry: type, text1 (label/des), text2 (subtitle), cat, sortby, sortdir.
 *
 * @return array<int,array<string,string>>
 */
function caHomeSections() {
	return [
		["type"=>"onlynew",       "text1"=>tr("Recently Added"),      "text2"=>tr("Check out these newly added applications from our awesome community"), "cat"=>"All",        "sortby"=>"FirstSeen",          "sortdir"=>"Down"],
